H5PEditor.RangeListScore = (function ($, TableList) {
   ...
   * @param {List} list
   */
  function RangeListScore(list) {
    var self = this;

    // Initialize inheritance
    TableList.call(self, list, 'h5p-editor-range-list-score');
    ...
    self.once('tableprepared', function (event) {
      ...
      // Apply text styling to number fields
      for (var i = 3; i < headRow.children.length; i++) {
        if (headRow.children[i].classList.contains('h5peditor-type-number')) {
          headRow.children[i].classList.remove('h5peditor-type-number');
          headRow.children[i].classList.add('h5peditor-type-text');
        }
      }

      // Create button to evenly distribute ranges
      distributeButton = createDistributeButton(
        H5PEditor.t('H5PEditor.RangeListScore', 'distributeButtonLabel'),
        H5PEditor.t('H5PEditor.RangeListScore', 'distributeButtonWarning'),
      ...
      self.messageArea.className = 'h5p-editor-range-list-score-message-area';
    ...
    var validateSequence = function () {
      ...
      // Display a message
      self.messageArea.innerText = error ? H5PEditor.t('H5PEditor.RangeListScore', 'rangeOutOfSequenceWarning') : '';
  ...
  RangeListScore.prototype = Object.create(TableList.prototype);
  RangeListScore.prototype.constructor = RangeListScore;

  return RangeListScore;
