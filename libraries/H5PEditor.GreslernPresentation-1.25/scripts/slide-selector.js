H5PEditor.GreslernPresentation.SlideSelector = (function ($, EventDispatcher) {

  /**
   * Create a Slide Selector with background settings
   *
   * @class H5PEditor.GreslernPresentation.SlideSelector
   * @extends H5P.EventDispatcher Enables pub/sub
   * @param {H5PEditor.GreslernPresentation} cpEditor CP editor for listening to events
  ...
  function SlideSelector(cpEditor, $slides, globalFields, slideFields, params) {
    ...
    var $title = $('<div class="h5p-background-selector-title">')
      .html(H5PEditor.t('H5PEditor.GreslernPresentation', 'slideBackground', {}))
      .appendTo($popup);
    $('<div>', {
    ...
    var singleSlideFields = H5PEditor.GreslernPresentation.findField('slideBackgroundSelector', slideFields.field.fields);
    ...
    var initBgSelectors = function () {
      ...
      // Global bg selector
      var templateString = H5PEditor.t('H5PEditor.GreslernPresentation', 'template');
      var currentSlideString = H5PEditor.t('H5PEditor.GreslernPresentation', 'currentSlide');
      $globalContent = createSlideSelector(templateString, true);
      globalBackground = new H5PEditor.GreslernPresentation.BackgroundSelector($slides.children())
        .addBgSelector(globalFields, params, $globalContent, { isVisible: true })
        .setDescription(H5PEditor.t('H5PEditor.GreslernPresentation', 'templateDescription', { ':currentSlide': currentSlideString }))
        .addResetButton();
      ...
      $slides.children().each(function (idx) {
        initSingleSlide($slideContent, idx)
          .setDescription(H5PEditor.t('H5PEditor.GreslernPresentation', 'currentSlideDescription', { ':template': templateString }))
          .addResetButton(H5PEditor.t('H5PEditor.GreslernPresentation', 'resetToTemplate'));
      });
    ...
    var addSlide = function (newSlideIndex) {
      ...
      initSingleSlide($slideContent, newSlideIndex)
        .setDescription(H5PEditor.t('H5PEditor.GreslernPresentation', 'currentSlideDescription', {
          ':template': H5PEditor.t('H5PEditor.GreslernPresentation', 'template')
        }))
        .addResetButton(H5PEditor.t('H5PEditor.GreslernPresentation', 'resetToTemplate'));
     ...
     * @returns {H5PEditor.GreslernPresentation.BackgroundSelector} Background selector that was created
     */
    var initSingleSlide = function ($wrapper, idx) {
      var slideParams = params.slides[idx];

      var singleSlide = new H5PEditor.GreslernPresentation.BackgroundSelector($slides.children().eq(idx), true);
     ...
     * @returns {H5PEditor.GreslernPresentation.SlideSelector}
     */
